...
export class TaskPlanner {
     ...
     */
    async createPlan(command: string, context: PlanningContext): Promise<Task> {
    ...
    async refinePlan(
        task: Task,
        feedback: { stepIndex: number; error?: string; observation?: string },
    ): Promise<Task> {
...
`;
        ...
        if (parsed.revisedSteps) {
            ...
            const newSteps = parsed.revisedSteps.map((step: any, idx: number) => ({
     ...
     */
    async decompose(command: string): Promise<string[]> {
...
`;
     ...
     */
    private buildPlanningPrompt(command: string, context: PlanningContext): string {
        return `
...
`;
    ...
    private parsePlanResponse(response: string): {
        ...
        const steps: TaskStep[] = parsed.steps.map((step: any, idx: number) => ({
            id: `step-${Date.now()}-${idx}`,
            description: step.description || '',
     ...
     */
    private normalizeStepType(type: string): TaskStep['type'] {
